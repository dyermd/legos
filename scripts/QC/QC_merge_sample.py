...
class QC_Sample:
	...
	def QC_merge_runs(self):
		...
		if self.sample_json['sample_type'] == 'germline':
			...
			if merge_dir != '':
				...
				self.merge_runs(passing_bams, merge_dir, 'germline', "Merged%d"%merge_count)
			...
			if self.sample_json['analysis']['settings']['type'] == 'all_tumor_normal':
				...
				if normal_merge_dir != '' or tumor_merge_dir != '' or (len(normal_runs) == 1 and len(tumor_runs) == 1):	
					...
					if 'final_normal_json' in self.sample_json and 'final_tumor_json' in self.sample_json:
						qc_json = self.QC_2Runs(normal_run, tumor_run, 'normal_', 'tumor_', '_merged')
					...
					# TODO need to update cleaning the 'merged' run
					#self.cleanup_sample.cleanup_runs(self.sample_json['runs'], self.sample_json['analysis']['settings']['cleanup'], self.no_errors)
	...
	def QC_2Runs(self, run1, run2, pref1, pref2, merged=''):
		...
		if 'results_QC_json' in self.sample_json:
			...
			output_folder = self.sample_json['qc_folder']
		elif 'output_folder' not in self.sample_json:
			output_json = "%s/results_QC.json"%self.sample_json['qc_folder']
			self.sample_json['results_QC_json'] = output_json
			output_folder = self.sample_json['qc_folder']
		else:
			# cleanup these paths
			output_json = "%s/results_QC.json"%self.sample_json['output_folder']
			output_folder = self.sample_json['output_folder']
			self.sample_json['results_QC_json'] = output_json
			self.sample_json['qc_folder'] = output_folder
			del self.sample_json['output_folder']
		...
		for chromosome in self.sample_json['analysis']['settings']['chromosomes_to_analyze'+merged]:
			...
			output_dir = "%s/%s%svs%s%s"%(self.sample_json['output_folder'], chromosome, run1_json['run_name'], run2_json['run_name'])
			comp_type = run1_json['run_type'] + "_" + run2_json['run_type']
	
			# If this QC comparison has already been made, skip it.
			if 'QC_comparisons' in output_json_data and chromosome in output_json_data['QC_comparisons'] and \
					comp_type in output_json_data['QC_comparisons'][chromosome] and run1vsrun2 in output_json_data['QC_comparisons'][chromosome][comp_type]:
	...
	def merge_runs(self, passing_bams, output_dir, run_type, run_name='', pref=''):
			...
			self.update_run_status(merger.merged_json)
